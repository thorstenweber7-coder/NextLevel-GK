...
function removePngWhiteBackground(inputPath, outputPath) {
  ...
  while (offset < fileBuf.length) {
    ...
    if (type === 'IHDR') {
      ...
      bitDepth = data[8];
      colorType = data[9]; // 6 for RGBA, 2 for RGB
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let i = 0; i < stride; i++) {
      ...
      let val = 0;
      if (filterType === 0) val = rawByte; // None
      else if (filterType === 1) val = (rawByte + a) & 0xff; // Sub
      else if (filterType === 2) val = (rawByte + b) & 0xff; // Up
      else if (filterType === 3) val = (rawByte + Math.floor((a + b) / 2)) & 0xff; // Average
      else if (filterType === 4) val = (rawByte + paethPredictor(a, b, c)) & 0xff; // Paeth
  ...
  // Flood fill from outer perimeter to identify background white pixels
  ...
  for (let y = 0; y < height; y++) {
    outputRawBuffer[y * outputScanlineLen] = 0; // Filter None
  ...
  // Helper to build a PNG chunk with CRC32
